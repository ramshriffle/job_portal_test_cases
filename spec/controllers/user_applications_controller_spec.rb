...
RSpec.describe UserProfilesController, type: :controller do
  ...
  let(:user) { FactoryBot.create(:user),type: 'JobSeeker'}
  let(:job) { FactoryBot.create(:job, user_id: user_id)}
  let(:user_profile) { FactoryBot.create(:user_application, user_id: user.id)}
  let(:token) do
    generate_token(user)
  end  
